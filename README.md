...
此外我認為微軟的巨集功能句難用且不直觀
版本依賴性問題也搞得亂七八糟
希望這套簡單到不行的專案可以幫助到需要的人



**為台灣社福機構的研習證明書產出需求設計**，任何機構都可以 fork 後依需求客製化使用。
...
要客製化給你的機構使用，最少需要動這幾個地方：

1. **機構名稱**：在 app 內「產生」分頁的「辦理單位」欄位填入完整名稱（已改為使用者輸入，不再寫死在程式碼）
2. **文號前綴**：搜尋 `src/index.html` 中的「南市社身字第」，改成你們的主管機關格式
3. **頂部抬頭**：搜尋「臺南市政府社會局」
